...
export const npp = (arrivalTime: number[], burstTime: number[], priorities: number[],  jobIds: string[]) => {
  ...
  const solvedProcessesInfo: { job: string; at: number; bt: number;
    ft: number; tat: number; wat: number; }[] = [];
  const readyQueue: { job: string; at: number; bt: number; priority: number; }[] = [];
